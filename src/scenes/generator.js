...
export default function LevelGenerator (config) {
  ...
  return class Level extends Phaser.Scene {
    ...
    update () {
      ...
      // First, check for triggers like death.
      ...
      // Then, update the things in the world.
      player.update(cursors, utils.getGravityDirection(this.physics.world.gravity));
    }
    ...
    createObjects() {
      ...
      if(config.text) {
        this.objects.text = this.add.text(400, 300 * 0.8, config.text, {
          font: "bold 30px Arial", fill: "#fff"
        }).setOrigin(0.5, 0.5);
      }
    }
